...
export default function DonationForm({ className = "", inlineName = false, returnPath = "/sponsors", onSponsorRecorded }) {
  ...
  async function handleDonate(event) {
    ...
    try {
      const response = await fetch(apiUrl("/api/donations/checkout-session"), {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          amount_cents: Math.round(Number(values.amount_major) * 100),
  ...
  return (
    <form className={className} onSubmit={handleDonate}>
      <h2>{t("donate.form.title")}</h2>
      <div className={`grid${inlineName ? " sponsor-donation-grid" : ""}`}>
        <SelectField
          values={values}
          setValues={setValues}
          name="currency"
        ...
        <TextField
          ...
          setValues={setValues}
          name="name"
          label={t("donate.form.publicName")}
          full={!inlineName}
        />
        <TextField
          values={values}
